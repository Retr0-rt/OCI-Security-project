...
# the bastion nsg configuration
###############################
resource "oci_core_network_security_group" "bastion_nsg" {
  compartment_id = var.compartment_id
  vcn_id         = oci_core_vcn.secure_vcn.id
  display_name   = "NSG_Bastion_St26"
}
...
resource "oci_core_network_security_group_security_rule" "bastion_egress_ssh" {
  ...
  destination               = oci_core_network_security_group.app_nsg.id
  stateless                 = false
  description               = "Enforce sercurity by allowing administrative access to private resources only through bastion server"
...
resource "oci_core_network_security_group_security_rule" "app_ingress_from_bastion" {
  ...
  stateless                 = false
  description               = "allow ssh traffic from the bastion server on port 22"
